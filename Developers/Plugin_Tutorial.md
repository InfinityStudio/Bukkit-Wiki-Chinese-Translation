...
现在，你的项目已经完全设置完毕，可以开始创建类来实现插件的功能。请注意：Plugin的类其实是JavaPlugin类的子类，所以请确保在你的项目中只有一个类直接或者间接的继承了JavaPlugin类。我们推荐使用与插件名相同的名称来命名你的主类。请右键点击上一步建立的包，然后点选_New > Class_来建立新的类。你建立完成的类应该看起来像这样：
